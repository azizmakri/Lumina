...
namespace LuminaApp.Infrastructure.Configurations
{
    public class UserConfiguration : IEntityTypeConfiguration<User>
    {
        public void Configure(EntityTypeBuilder<User> builder)
        {
            ...
            builder.HasMany(student=>student.StudentsEvaluations).WithOne(evaluation=>evaluation.Student).HasForeignKey(evaluation=>evaluation.StudentFk).OnDelete(DeleteBehavior.NoAction);
            builder.HasMany(teacher=> teacher.TeachersEvaluations).WithOne(evaluation=>evaluation.Teacher).HasForeignKey(evaluation=>evaluation.TeacherFk).OnDelete(DeleteBehavior.NoAction);
